...
__HELP__ = f"""
<blockquote><b>[9] {sc("alquran board")}</b>

<code>{{0}}alquran nomor_surah</code>
<code>{{0}}alquran nomor_surah:ayat</code>
<code>{{0}}surah nama</code>
{sc("membuka surah atau ayat alquran dengan tampilan yang lebih rapi")}</blockquote>
"""


def quran_header(title, subtitle):
    return (
        f"<blockquote><b>[9] {sc('alquran center')}</b></blockquote>\n"
        f"<blockquote>surah: <code>{title}</code>\n"
        f"detail: <code>{subtitle}</code></blockquote>"
    )
...
async def alquran(client, message):
    ...
    wait = await message.reply_text(
        f"<blockquote><b>{sc('mencari data alquran')}...</b></blockquote>"
    )
    try:
        if ":" in arg and message.command[0].lower() == "alquran":
            surah_no, ayat_no = [x.strip() for x in arg.split(":", 1)]
            data = requests.get(
                f"https://equran.id/api/v2/surat/{surah_no}", timeout=30
            ).json()
            surat = data.get("data") or {}
            ayat = next(
                (
                    x
                    for x in surat.get("ayat", [])
                    if str(x.get("nomorAyat")) == ayat_no
                ),
                None,
            )
            if not ayat:
                return await wait.edit(
                    f"<blockquote><b>{sc('ayat tidak ditemukan')}.</b></blockquote>"
                )
            text = (
                ...
                + f"<blockquote><b>[9] {sc('ayat spotlight')}</b></blockquote>"
            )
        ...
        if message.command[0].lower() == "surah" and not arg.replace(" ", "").isdigit():
            ...
            if not found:
                return await wait.edit(
                    f"<blockquote><b>{sc('surah tidak ditemukan')}.</b></blockquote>"
                )
        ...
        text = (
            ...
            + "\n"
            + f"<blockquote>tempat turun: <code>{surat.get('tempatTurun', '-')}</code>\n"
            + f"audio: <a href='{surat.get('audioFull', {}).get('05', '')}'>{sc('putar murattal')}</a></blockquote>\n"
            + "<blockquote>"
            + "\n\n".join(ayat_lines)
            + "</blockquote>\n"
            + f"<blockquote><b>[9] {sc('preview 5 ayat pertama')}</b></blockquote>"
        )
        await wait.edit(text, disable_web_page_preview=True)
    except Exception as error:
        await wait.edit(
            f"<blockquote><b>{sc('gagal mengambil data alquran')}.</b>\n<code>{error}</code></blockquote>"
        )
